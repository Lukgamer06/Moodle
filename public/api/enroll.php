...
if ($method === 'POST') {
    ...
    if (!empty($emails)) {
        ...
        /*
          Aquí agregamos u.nrc para que cuando se matricule masivamente
          también pueda devolverse el NRC del estudiante agregado.
        */
        $find = $pdo->prepare("
            SELECT id, name, email, nrc 
            FROM users 
            WHERE email = ? 
              AND role = 'student'
        ");
...
if ($method === 'GET') {
    ...
    /*
      ...
      antes solo traía id, name y email.
      Ahora también trae nrc.
    */
    $stmt = $pdo->prepare("
        SELECT 
            u.id, 
